  ...
  const fetchUserProfile = async () => {
    try {
      ...
      setFormData({
        ...
        resume: "uploads/resume-1754193190283-787090974.pdf",
        profileCompleted: true,
        createdAt: "2025-08-03T03:53:14.300Z"
      };
      
      // Check if credentials match the demo user
      if (email === "chaitanyapawar410@gmail.com" && password === "password") {
        localStorage.setItem('token', 'demo-token');
        localStorage.setItem('user', JSON.stringify(demoUser));
        setUser(demoUser);
      } else {
        throw new Error('Invalid credentials. Use: chaitanyapawar410@gmail.com / password');
      }
        email: user?.email || '',
        phone: user?.phone || '',
        dateOfBirth: user?.dateOfBirth ? new Date(user.dateOfBirth).toISOString().split('T')[0] : '',
